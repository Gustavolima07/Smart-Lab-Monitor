...
export default function GerenciarAcessosScreen() {
  ...
  useEffect(() => {
    ...
    const unsubPermissoes = onValue(
      userPermsRef,
      carregarAmbientesGerenciáveis,
    );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(usuariosRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        Object.keys(usuariosData).forEach((uid) => {
          ...
          const permissao = userData.ambientesPermitidos
            ? userData.ambientesPermitidos[ambienteSelecionado.id]
            : null;
          ...
          if (permissao) {
            ...
            let cargo = "ti";
            if (
              uid === ambienteSelecionado.criadorUid ||
              permissao === "owner"
            ) {
              cargo = "dono";
    ...
    ambienteSelecionado?.criadorUid,
    user?.uid,
    user?.photoURL,
  ]);
  ...
  const handleAdicionarAcesso = async () => {
    ...
    try {
      ...
      const updates = {};
      updates[
        `usuarios/${targetUid}/ambientesPermitidos/${ambienteSelecionado.id}`
      ] = "ti";
      ...
      setSuccessMessage(
        `Acesso ao ambiente "${ambienteSelecionado.nome}" concedido para ${targetNome} (Cargo: TI)!`,
      );
      setEmailInput("");
    } catch (error) {
  ...
  const handleAlterarCargo = async (targetUid, targetNome, novoCargo) => {
    ...
    const executarAlteracao = async () => {
      try {
        ...
        setSuccessMessage(
          `Cargo de ${targetNome} alterado para ${cargoNomeExtenso} com sucesso!`,
        );
        setErrorMessage("");
      } catch (error) {
    ...
    if (Platform.OS === "web") {
      ...
      Alert.alert("Alterar Cargo", confirmMsg, [
  ...
  const handleRemoverAcesso = async (targetUid, targetEmail) => {
    ...
    const executarRemocao = async () => {
      try {
        ...
        setSuccessMessage(
          `Acesso ao ambiente "${ambienteSelecionado.nome}" removido.`,
        );
      } catch (error) {
    ...
    if (Platform.OS === "web") {
      ...
      Alert.alert("Confirmar Remoção", confirmMsg, [
        { text: "Cancelar", style: "cancel" },
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.title, { marginTop: 15, textAlign: "center" }]}>
          Nenhum Ambiente Sob Sua Gestão
        </Text>
        <Text style={[styles.subtitle, { textAlign: "center" }]}>
          Você só pode gerenciar acessos dos ambientes onde você é Dono ou
          Administrador.
        </Text>
      </MotiView>
    );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Adicione colaboradores, defina cargos (Admin/TI) ou remova permissões.
        </Text>
      </MotiView>
      ...
      >
        ...
        <Text style={styles.activeBannerText}>
          Gerenciando colaboradores do ambiente:{" "}
          <Text style={styles.activeBannerBold}>
      ...
      <Text style={styles.listTitle}>
        Usuários com Acesso em{" "}
        <Text style={{ color: "#005b9f" }}>{ambienteSelecionado.nome}</Text>:
      </Text>
            ...
            <View style={styles.userInfo}>
              ...
              <View style={styles.badgesRow}>
                <View style={styles.ambienteBadge}>
                  ...
                  <Text style={styles.ambienteBadgeText}>
                    {ambienteSelecionado.nome}
                  </Text>
                </View>
                ...
                {item.cargo === "dono" && (
                  ...
                  >
                    ...
                    <Text style={[styles.roleBadgeText, { color: "#f57f17" }]}>
                      Dono
                    </Text>
                  </View>
                )}
                ...
                {item.cargo === "admin" && (
                  ...
                  >
                    <Ionicons
                      ...
                      color="#3f51b5"
                    />
                    <Text style={[styles.roleBadgeText, { color: "#3f51b5" }]}>
                      Admin
                    </Text>
                  </View>
                )}
                ...
                {item.cargo === "ti" && (
                  ...
                  >
                    <Ionicons
                      ...
                      color="#00695c"
                    />
                    <Text style={[styles.roleBadgeText, { color: "#00695c" }]}>
                      TI
                    </Text>
                  </View>
                )}
                ...
                {item.cargo === "admin" ? (
                  <ZoomCard>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleAlterarCargo(item.uid, item.nome, "ti")
                      }
                    >
                      <Ionicons
                  ...
                  <ZoomCard>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleAlterarCargo(item.uid, item.nome, "admin")
                      }
                    >
                      <Ionicons
        ...
        ListEmptyComponent={
          ...
          >
            <Text style={styles.emptyText}>
              Nenhum outro colaborador vinculado a este ambiente.
            </Text>
          </MotiView>
        }
      />
