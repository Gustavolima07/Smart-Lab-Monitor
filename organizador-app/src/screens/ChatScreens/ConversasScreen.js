...
export default function ConversasScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const meusAmbientesKeys = meusDados?.ambientesPermitidos ? Object.keys(meusDados.ambientesPermitidos) : [];

    // Estrutura em Seções por Ambiente
    const secoes = meusAmbientesKeys.map((ambId) => {
      const ambData = ambientesRaw[ambId];
      const nomeAmbiente = ambData?.nome || ambData?.nomeAmbiente || ambData?.titulo || ambId;
  ...
  const secoesFiltradas = secoesContatos.map(section => {
    ...
    const contatosFiltrados = section.data.filter(item => {
      ...
      const msgMatch = item.ultimaMensagem && item.ultimaMensagem.toLowerCase().includes(query);
      ...
      <SectionList
        ...
        renderSectionHeader={({ section: { title } }) => (
          <View style={styles.sectionHeader}>
            <Ionicons name="business" size={16} color="#005b9f" style={{ marginRight: 6 }} />
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>
            {searchText ? 'Nenhuma conversa encontrada.' : 'Nenhum colega encontrado nos seus ambientes.'}
          </Text>
        }
      />
    </SafeAreaView>
