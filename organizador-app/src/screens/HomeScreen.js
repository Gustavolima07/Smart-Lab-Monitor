...
export default function HomeScreen({ navigation }) {
  ...
  const handleCopiarId = async (id, e) => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        Alert.alert(
          "Copiado!",
          `ID (${id}) copiado para a área de transferência.`,
        );
      }
    } catch (error) {
  ...
  const handleCriarAmbiente = async () => {
    if (!nomeNovoAmbiente.trim()) {
      if (Platform.OS === "web")
        window.alert("Digite um nome válido para o ambiente.");
    ...
    try {
      ...
      await set(
        ref(db, `usuarios/${user.uid}/ambientesPermitidos/${novoAmbId}`),
        true,
      );
  ...
  const handleExcluirAmbiente = async (ambId, ambNome) => {
    const executarExclusao = async () => {
      try {
        ...
        await remove(
          ref(db, `usuarios/${user.uid}/ambientesPermitidos/${ambId}`),
        );

        if (Platform.OS === "web")
          window.alert("Ambiente excluído com sucesso!");
    ...
    if (Platform.OS === "web") {
      const confirm = window.confirm(
        `Deseja realmente excluir o ambiente "${ambNome}"?`,
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const nomeAmbiente =
            item.nome || item.escola?.nome || "Ambiente sem nome";

          return (
            <ZoomCard style={styles.ambienteCardWrapper}>
             
              <TouchableOpacity
                style={styles.ambienteCard}
                onPress={() =>
                  navigation.navigate("LabList", { ambienteData: item })
                }
                >
                ...
                <View style={styles.cardContent}>
                  ...
                  <Text style={styles.ambienteHint}>
                    Toque para acessar os laboratórios
                  </Text>
                </View>
                <MaterialCommunityIcons
                  name="chevron-right"
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>
            Nenhum ambiente encontrado ou liberado para você.
          </Text>
        }
      />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Insira o nome do local (Ex: Escola Central)
            </Text>
